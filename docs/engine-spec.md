...
*   A `ProcessPoolExecutor` (sized to the CPUs the container may use, `ns_db.available_cpus()`, overridable via `--workers`) parallelizes hashing and metadata extraction across CPU cores.
...
*   **Tuning:**
    *   `--workers <N>` — overrides the `ProcessPoolExecutor` worker count (default: the CPUs the container may use: the host's cores reduced by a `--cpuset-cpus` set or a `--cpus` quota, rounded down, at least 1; `ns_db.available_cpus()`).
    *   `--exts <.ext1,.ext2,...>` — overrides the default extension set for directory scanning. Has no effect on `--file-ids` targeting.
*   **A photo's name is not enough (maintainer's rule):** a file with a photo format's extension whose content is not an image is logged and left alone. The scan judges content as ExifTool identified it (`not_an_image`): an empty file, an ExifTool error, or a MIME type that is not `image/…` is recorded `Failed` with a reason starting **"Not an image"**, so it appears in the log and the Error Center, and Copy and Move never take it. A PNG named `.jpg` is still an image and is catalogued. With no ExifTool answer for the file nothing is concluded, so a photo is never refused on a guess. An extension the user selected that is not a photo format (`.mov`) is outside the rule and keeps its own warning (`webui-spec.md` §3.2).
