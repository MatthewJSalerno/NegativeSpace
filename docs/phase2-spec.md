...
1. **Flat, or subdivided? `Undated/YYYY/`, with the year fixed by the content rather than by walk order.** At ~8%, a consolidated library could put tens of thousands of files in one directory — navigable by a tool, tedious for a person, which defeats the "easy to find" half of the rationale. So subdivide by year: the mtime is a real fact about the *file* even when it is not a fact about the *photograph*, so it can organise the folder without the tree ever claiming it is the date taken.
   ...
   **The year is the earliest usable `file_mtime` among every row sharing that content's `sha1_hash`** — not the placed row's own mtime. Both values are already catalogued (`photos.file_mtime`, `photos.sha1_hash`), so this is one aggregate over an existing column: no new field, and no reading through `metadata_json`. Taking the group's minimum rather than the placed row's own value is the whole point — every copy of that content computes the same year, so which copy the unsorted walk happened to reach first stops deciding anything, and the same library yields the same tree on every run.

   **"Usable" excludes a `file_mtime` of `0.0`**, which is what a failed `stat()` records — the column is not NULL in that case, so a group containing one unstattable file would otherwise take its minimum from the epoch and file the photo under `Undated/1970/`. Where no row in a group has a usable mtime there is no year to sort under, and the content belongs directly in `Undated/`; the folder must therefore tolerate loose files beside its year folders.

2. **What does `date_taken` hold?** **Recommend leaving it exactly as today** — the mtime, marked `date_source = 'file_mtime'` — and changing only *placement*. That keeps the existing census query working (`date_source` already distinguishes them), keeps a value in hand for when a real date is recovered, and confines the change to one decision in the path builder rather than spreading through the metadata layer. One consequence to state plainly rather than leave to be rediscovered: a photo's **folder year and its own `date_taken` can disagree**, because the folder reflects the group's earliest usable mtime while the row keeps what was read from that file. This is intended. The folder answers "where do I go to find this content"; the row records what this particular file said.

**Consequence worth recording:** this dissolves the undated-duplicate tiebreak — not by removing the choice, but by making it independent of who makes it. Two byte-identical copies with different mtimes previously produced different `date_taken` values, so the arbitrary anchor — whichever the unsorted walk reached first — decided which date folder the content landed in, and a later rename could not fix it. Under the rule above the year is derived from the content group rather than from the placed row, so every copy computes the same answer and the anchor's identity stops mattering. **An earlier draft of this section claimed the leak closed because there was "no date folder to choose".** That was written against a flat `Undated/` and is simply false under the recommended subdivision: with `Undated/YYYY/` there is still a folder to choose, which is exactly why the selection rule has to be deterministic. The leak does still survive for users who keep modification-time filing, which files by each row's own mtime into ordinary date folders — demoting it, for them, from a design flaw to a documented consequence of choosing that setting.
