...
| 10 | The **intent marker survives a power cut**: the move/copy loop commits at `synchronous=FULL`, so `status=Processing` with `dest_path` is fsynced before the source is unlinked | `get_db_connection`, `project-spec.md` §4.2 | **Enforced**, tested (`the move loop commits at full synchronous`). Scoped to that one connection: the scan path keeps `NORMAL`. Measured 1.42x on the move path (+1.9 ms/photo) versus the ~4.4x `NORMAL` buys on the scan path. Losing the marker was reproduced by probe — it leaves a delivered photo recorded `Failed`. |

### Outstanding

- [ ] **The audit log is less durable than the act it records — on the SCAN path only.** The move/copy half is closed: that loop now commits at `synchronous=FULL` (claim 10), so its intent markers and its audit rows are fsynced alongside the file operations they describe. What remains is the scan path, which batches a hundred rows per commit at `NORMAL` because that is where the ~4.4x lives: a power cut can still lose recently recorded discovery failures and scan results. Lower stakes — those rows describe reading, not deleting, and a re-Index reproduces them — but it is not nothing, since `operations` is the only non-derived table (`phase3-spec.md` §3.2). Decide between accepting it and saying so plainly here, or fsyncing the WAL once at run end, which would make a *completed* run's history durable without touching per-commit cost. **Not** a case for `FULL` globally: that buys back the 4.4x on the one path it was measured against.
